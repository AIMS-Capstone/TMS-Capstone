<x-transaction-form-section>
    ...
    <!-- Form Header Title -->
    <x-slot:description>
        <h1 class="text-3xl font-bold text-blue-900">Edit Journal</h1>
    </x-slot:description>
    ...
    <x-slot:form>
        ...
        <div class="grid grid-cols-4 gap-6 mb-4">
            ...
            <div class="col-span-2 bg-blue-50 p-4 rounded-tr-sm">
                ...
                <x-transaction-input id="total_amount" type="text" class="mt-1 block w-full bg-blue-50 text-blue-900 font-bold border-0" value="{{ $totalAmount }}" wire:model.defer="totalAmount" readonly />
            </div>
        </div>
            ...
            <table class="table-auto w-full text-left text-sm text-neutral-600">
                ...
                <tbody>
                    @foreach($journalRows as $index => $row)
                        <livewire:journal-row :key="$index" :index="$index" :journal-row="$row" :type="$type" :mode="$mode" />
                    @endforeach
                </tbody>
            </table>
        </div>
