    ...
    <x-slot:form>
        <div class="grid grid-cols-5 gap-6">
            ...
            <div class="mt-5 mb-8">
                ...
                <x-transaction-input id="date" type="date" class="mt-7  w-full" wire:model.defer="date" />
            </div>

            <!-- Invoice Number Field -->
            <div class="mt-5 mb-8">
                <x-transaction-label for="inv_number" :value="__('Invoice Number')"   required="true"/>
                <x-transaction-input id="inv_number" type="text" class="mt-7 w-full" wire:model.defer="inv_number" />
            </div>

            <!-- Reference Field -->
            <div class="mt-5 mb-8">
                <x-transaction-label for="reference" :value="__('Reference')" />
                <x-transaction-input id="reference" type="text" class="mt-7 w-full" wire:model.defer="reference" />
            </div>
            ...
            <div class="col-span-1 bg-blue-50 p-4 rounded-tr-sm">
                ...
                <x-transaction-input 
                    id="total_amount" 
                    type="text" 
                    class="font-bold text-blue-900 bg-blue-50 mt-7 w-full" 
                    value="{{ $totalAmount }}" 
            ...
            <table class="table-auto w-full text-left text-sm text-neutral-600">
                <thead class="bg-neutral-100 text-neutral-900">
                    <tr>
                        ...
                        <th class="px-4 py-2">Alphanumeric Tax Code (ATC)</th>
                        <th class="px-4 py-2">Chart of Account (COA)</th>
    ...
    $(document).ready(function() {
        $('#select_contact').select2({
            templateResult: function(data) {
                ...
                var $result = $(`<span>${data.text}<small style="display:block; font-size:15px; line-height:1; margin-top:0;">${$(data.element).data('tax-id')}</small></span>`);
                return $result;
            },
            templateSelection: function(data) {
                ...
                var $selection = $(`<span>${data.text}<span style="display:block; font-size:15px;">${$(data.element).data('tax-id')}</span></span>`);
                return $selection;
            },
            language: {
                noResults: function() {
        ...
        $('#select_contact').on('change', function() {
            $wire.set('selectedContact', $(this).val());
        });
