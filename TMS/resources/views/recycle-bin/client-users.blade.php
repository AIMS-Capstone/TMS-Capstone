<x-organization-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden sm:rounded-xs">
                ...
                <div class="overflow-x-auto pt-6 mt-4 px-10">
                    ...
                    <!-- Tabs Navigation -->
                    @php
                        $activeTab = request()->query('tab', 'client');
                    @endphp
                            ...
                            <div x-data="recycleBinHandler">
                                ...
                                <div class="flex flex-row items-center">
                                    <div class="relative w-80 p-5">
                                        <form x-target="client-table" action="{{ route('recycle-bin.organization.index') }}" method="GET" role="search" aria-label="Table" autocomplete="off">
                                            <input 
                                            type="search" 
                                            name="search" 
                                ...
                                <div class="my-4 overflow-y-auto max-h-[500px]">
                                    <table class="min-w-full bg-white" id="client-table">
                                        ...
                                        <tbody class="divide-y divide-zinc-200 text-sm text-zinc-700">
                                            @if($trashedClientUsers->isEmpty())
                                                <tr>
                                                    <td colspan="7" class="text-center p-4">
                                                        <img src="{{ asset('images/Box.png') }}" alt="No data available" class="mx-auto w-48 h-48" />
                                                        <h1 class="font-extrabold text-zinc-700">No Deleted Client Users yet</h1>
                                            ...
                                            @else
                                                @foreach($trashedClientUsers as $clientUser)
                                                    <tr class="hover:bg-slate-100 cursor-pointer ease-in-out">
                                                        <td class="py-3 px-4">
                                                            <input 
                                                                type="checkbox" 
                                                                :checked="selectedRows.includes(@json($clientUser->id))" 
                                                                @click="toggleCheckbox(@json($clientUser->id))" 
                                                                aria-label="Select item {{ $clientUser->registration_name }}" 
                                                                data-client-user-id="{{ $clientUser->id }}"
                                                            >
                                                        </td>
                                                        <td>
                                                            <button 
                                                            ...
                                                            x-on:click="$dispatch('open-view-client-modal', {
                                                                id: '{{ $clientUser->id }}', 
                                                                name: '{{ $clientUser->orgSetup->registration_name ?? 'N/A' }}',
                                                                tin: '{{ $clientUser->orgSetup->tin ?? 'N/A' }}', 
                                                                classification: '{{ $clientUser->orgSetup->type ?? 'N/A' }}', 
                                                                date_created: '{{ $clientUser->created_at->format('F j, Y') }}', 
                                                                role: 'Client'
                                                            })"
                                                            class="text-left py-4 px-4 font-bold underline hover:text-blue-600">{{ $clientUser->orgSetup->registration_name ?? 'N/A' }}
                                                            </button>
                                                        </td>
                                                        <td class="py-3 px-4">{{ $clientUser->orgSetup->tin }}</td>
                                                        <td class="text-left py-3 px-4">
                                                            @if ($clientUser->type) 
                                                                <span class="bg-yellow-100 text-zinc-800 text-xs font-medium me-2 px-4 py-2 rounded-full">Client</span>
                                                            @else
                                                                <span class="bg-zinc-100 text-zinc-800 text-xs font-medium me-2 px-4 py-2 rounded-full">No Account Yet</span>
                                                            @endif
                                                        </td>
                                                        <td class="p-4">{{ \Carbon\Carbon::parse($clientUser->deleted_at  )->format('F d, Y') ?? 'N/A' }}</td>
                                                        <td class="py-3 px-4">{{ $clientUser->deletedByUser->name ?? 'Unknown' }}</td>
                                                    </tr>
                                                @endforeach
                                        </tbody>
                                    </table>
                                    <div class="mt-4">
                                        {{ $trashedClientUsers->links('vendor.pagination.custom') }}
                                    </div>
                                    @endif
                                </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('recycleBinHandler', () => ({
                ...
                toggleAll() {
                    ...
                    this.selectedRows = this.checkAll ? @json($trashedClientUsers->pluck('id')) : [];
                },

                bulkRestore() {
                    axios.post('{{ route("recycle-bin.client-users.bulkRestore") }}', { ids: this.selectedRows })
                        .then(response => location.reload())
                        .catch(error => console.error(error));
                },

                bulkDelete() {
                    axios.post('{{ route("recycle-bin.client-users.bulkDelete") }}', { ids: this.selectedRows, _method: "DELETE" })
                        .then(response => location.reload())
                        .catch(error => console.error(error));
                },
    ...
    function setEntries(entries) {
        ...
        form.method = 'GET';
        form.action = "{{ route('recycle-bin.client-users.index') }}";
