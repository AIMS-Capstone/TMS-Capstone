<x-organization-layout>
                    ...
                    <hr class="mx-1 mt-auto">
                                    ...
                                    x-data="{
                                        ...
                                        toggleCheckbox(id) {
                                            ...
                                            this.checkAll = this.selectedRows.length === {{ json_encode($trashedEmployees->count()) }};
                                        },
                                        toggleAll() {
                                            this.checkAll = !this.checkAll;
                                            if (this.checkAll) {
                                                this.selectedRows = {{ json_encode($trashedEmployees->pluck('id')->toArray()) }}; 
                                            } else {
                                        ...
                                        deleteRows() {
                                            ...
                                            fetch('{{ route('recycle-bin.employees.bulkDelete' ) }}', {
                                                method: 'DELETE',
                                                headers: {
                                        ...
                                        restoreRows() {
                                            ...
                                            fetch('{{ route('recycle-bin.employees.bulkRestore' ) }}', {
                                                method: 'POST',
                                                headers: {
                                                    'Content-Type': 'application/json',
                                                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                            ...
                            <div x-data="recycleBinHandler">
                                ...
                                <div class="flex flex-row items-center">
                                    ...
                                    <div class="relative w-80 p-5">
                                        <form x-target="employees-table" action="/recycle-bin/employees" role="search" aria-label="Table" autocomplete="off">
                                            <input 
                                            type="search" 
                                            name="search"
                                                    ...
                                                    <tr class="hover:bg-slate-100 cursor-pointer ease-in-out">
                                                        ...
                                                        <!-- Body Checkbox for Individual Selection -->
                                                            <label x-show="showCheckboxes" class="flex items-center cursor-pointer text-neutral-600" x-cloak>
                                                                <div class="relative flex items-center">
                                                                    <input type="checkbox" @click="toggleCheckbox('{{ $employee->id }}')" :checked="selectedRows.includes('{{ $employee->id }}')" id="employee{{ $employee->id }}" 
                                                        ...
                                                        <td>
                                                            <button x-data 
                                                                x-on:click="$dispatch('open-view-employee-modal', { id: '{{ $employee->id }}', first_name: '{{ $employee->first_name}} {{ $employee->last_name}}', 
                                                                tin: '{{ $employee->tin }}', date_of_birth: '{{ $employee->date_of_birth }}', contact_number: '{{ $employee->contact_number }}', nationality: '{{ $employee->nationality }}', 
                                                                address: '{{ $employee->address }}', zip_code: '{{ $employee->zip_code }}'})"
                                                        ...
                                                        <td class="p-4">{{ \Carbon\Carbon::parse($employee->deleted_at)->format('F d, Y h:i A') ?? 'N/A' }}</td>
                                ...
                                {{-- View Details --}}
                                <div x-data="{ showEmp: false, employee: {} }" x-show="showEmp"
                                    @open-view-employee-modal.window="showEmp = true; employee = $event.detail" x-on:close-modal.window="showEmp = false"
                                    x-effect="document.body.classList.toggle('overflow-hidden', showEmp)" class="fixed z-50 inset-0 flex items-center justify-center m-2 px-6" x-cloak>
                                    <!-- Modal background -->
                                        ...
                                        <div class="p-10">
                                            <div class="mb-5 flex justify-between items-start">
                                                <div class="w-2/3 pr-4">
                                                    <label class="block text-sm font-bold text-zinc-700">Employee Name</label>
                                                    ...
                                                    x-bind:value="`${employee.first_name || ''} ${employee.last_name || ''}`.trim()" disabled readonly>
                                                </div>
                                                <div class="w-2/3 text-left">
                                                    ...
                                                    <input class="peer py-3 pe-0 block w-full font-light bg-transparent border-t-transparent border-b-1 truncate border-x-transparent border-b-gray-200 text-sm focus:border-b-gray-200"
                                                        x-bind:value="employee.tin" disabled readonly>
                                                </div>
                                            </div>
                                            <div class="mb-5 flex justify-between items-start">
                                                <div class="w-2/3 pr-4">
                                                    <label class="block text-sm font-bold text-zinc-700">Date of Birth</label>
                                                    <input class="peer py-3 pe-0 block w-full font-light bg-transparent border-t-transparent border-b-1 border-x-transparent border-b-gray-200 text-sm focus:border-b-gray-200"
                                                    x-bind:value="new Date(employee.date_of_birth).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })" disabled readonly>
                                                </div>
                                                <div class="w-2/3 pr-4">
                                                    <label class="block text-sm font-bold text-zinc-700">Contact</label>
                                                    <input class="peer py-3 pe-0 block w-full font-light bg-transparent border-t-transparent border-b-1 border-x-transparent border-b-gray-200 text-sm focus:border-b-gray-200"
                                                        x-bind:value="employee.contact_number" disabled readonly>
                                                </div>
                                            </div>
                                            <div class="mb-5 flex justify-between items-start">
                                                <div class="w-2/3 pr-4">
                                                    <label class="block text-sm font-bold text-zinc-700">Nationality</label>
                                                    <input class="peer py-3 pe-0 block w-full font-light bg-transparent border-t-transparent border-b-1 border-x-transparent border-b-gray-200 text-sm focus:border-b-gray-200"
                                                        x-bind:value="employee.nationality" disabled readonly>
                                                </div>
                                                <div class="w-2/3 text-left">
                                                    <label class="block text-sm font-bold text-zinc-700">Address</label>
                                                    <input class="peer py-3 pe-0 block w-full font-light bg-transparent border-t-transparent border-b-1 border-x-transparent border-b-gray-200 text-sm focus:border-b-gray-200"
                                                        x-bind:value="employee.address" disabled readonly>
                                                </div>
                                                <div class="w-2/3 space-x-2 text-left">
                                                    <label class="block text-sm font-bold text-zinc-700">Zip Code</label>
                                                    <input class="peer py-3 pe-0 block w-full font-light bg-transparent border-t-transparent border-b-1 border-x-transparent border-b-gray-200 text-sm focus:border-b-gray-200"
                                                        x-bind:value="employee.zip_code" disabled readonly>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="bg-white rounded-lg shadow-lg p-6 max-w-md w-full relative">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <!-- Title -->
                                        <h2 class="text-2xl font-extrabold text-blue-600 mb-2">Restore Employee(s)</h2>

                                        <!-- Description -->    
                                        <p class="text-sm text-zinc-700 text-center">You're going to restore the selected employee(s) in the Recycle Bin table. Are you sure?</p>
                                ...
                                <div 
                                    ...
                                    <div class="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full relative">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <!-- Title -->
                                            <h2 class="text-2xl font-extrabold text-zinc-800 mb-2">Delete Employee(s)</h2>

                                            <!-- Description -->
                                            <p class="text-sm text-zinc-600 text-center">
                                                You're going to delete permanently the selected employee(s) in the Employee Recycle Bin table. Are you sure?
                                            </p>
    ...
    function setEntries(entries) {
        ...
        form.method = 'GET';
        form.action = "{{ route('recycle-bin.employees.index') }}";
