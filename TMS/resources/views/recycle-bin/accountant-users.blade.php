<x-organization-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden sm:rounded-xs">
                ...
                <div class="overflow-x-auto pt-6 mt-4 px-10">
                    ...
                    <!-- Tabs Navigation -->
                    @php
                        $activeTab = request()->query('tab', 'accountant');
                    @endphp
                            ...
                            <div x-data="recycleBinHandler">
                                ...
                                <div class="flex flex-row items-center">
                                    <div class="relative w-80 p-5">
                                        <form x-target="accountant-table" action="{{ route('recycle-bin.accountant-users.index') }}" method="GET" role="search" aria-label="Table" autocomplete="off">
                                            <input 
                                            type="search" 
                                            name="search" 
                                ...
                                <div class="my-4 overflow-y-auto max-h-[500px]">
                                    <table class="min-w-full bg-white" id="accountant-table">
                                        ...
                                        <tbody class="divide-y divide-zinc-200 text-sm text-zinc-700">
                                            @if($trashedAccountantUsers->isEmpty())
                                                <tr>
                                                    <td colspan="7" class="text-center p-4">
                                                        <img src="{{ asset('images/Box.png') }}" alt="No data available" class="mx-auto w-48 h-48" />
                                                        <h1 class="font-extrabold text-zinc-700">No Deleted Accountant Users yet</h1>
                                            ...
                                            @else
                                                @foreach($trashedAccountantUsers as $AccountantUser)
                                                    <tr class="hover:bg-slate-100 cursor-pointer ease-in-out">
                                                        <td class="py-3 px-4">
                                                            <input type="checkbox" :checked="selectedRows.includes(@json($AccountantUser->id))" 
                                                                @click="toggleCheckbox(@json($AccountantUser->id))" aria-label="Select item {{ $AccountantUser->name }}" data-accountant-user-id="{{ $AccountantUser->id }}">
                                                        </td>
                                                        <td>
                                                            <button x-data 
                                                                x-on:click="$dispatch('open-view-user-modal', { id: '{{ $AccountantUser->id }}', name: '{{ $AccountantUser->name }}', email: '{{ $AccountantUser->email }}', date_created: '{{ $AccountantUser->created_at->format('F j, Y') }}', role: '{{ $AccountantUser->role }}' })"
                                                                class="text-left py-4 px-4 font-bold underline hover:text-blue-600">
                                                                {{ $AccountantUser->name }}
                                                            </button>
                                                        </td>
                                                        <td class="py-3 px-4">{{ $AccountantUser->email }}</td>
                                                        <td class="text-left py-3 px-4">
                                                            @if ($AccountantUser->role === 'Admin') 
                                                                <span class="bg-zinc-100 text-zinc-800 text-xs font-medium me-2 px-4 py-2 rounded-full">Admin</span>
                                                            @else
                                                                <span class="bg-zinc-100 text-zinc-800 text-xs font-medium me-2 px-4 py-2 rounded-full">Accountant</span>
                                                            @endif
                                                        </td>
                                                        <td class="p-4">{{ \Carbon\Carbon::parse( $AccountantUser->deleted_at   )->format('F d, Y') ?? 'N/A' }}</td>
                                                        <td class="py-3 px-4">{{ $AccountantUser->deletedByUser->name ?? 'Unknown' }}</td>
                                                    </tr>
                                                @endforeach
                                        </tbody>
                                    </table>
                                    <div class="mt-4">
                                        {{ $trashedAccountantUsers->links('vendor.pagination.custom') }}
                                    </div>
                                    @endif
                                </div>
                                ...
                                <!-- Modals -->
                                {{-- View Details --}}
                                <div x-data="{ showUser: false, user: {} }" x-show="showUser"
                                    @open-view-user-modal.window="showUser = true; user = $event.detail" x-on:close-modal.window="showUser = false"
                                    x-effect="document.body.classList.toggle('overflow-hidden', showUser)" class="fixed z-50 inset-0 flex items-center justify-center m-2 px-6" x-cloak>
                                    <!-- Modal background -->
                                ...
                                <div x-show="showConfirmDeleteModal" class="fixed inset-0 bg-gray-200 z-50 bg-opacity-50 flex justify-center items-center" @click.away="showConfirmDeleteModal = false"
                                    ...
                                    <div class="bg-white p-10 rounded-lg shadow-lg max-w-lg w-full relative">
                                        ...
                                        <h2 class="text-xl text-zinc-700 font-bold text-start mb-4">Permanently Delete Account(s)</h2>
                                        <p class="text-start mb-6 text-sm text-zinc-700">Are you sure you want to permanently delete the selected account(s) to the recycle bin?</p>
                                        <div class="bg-red-100 border-l-8 border-red-500 text-red-500 p-6 rounded-lg mb-6">
                                            <ul class="list-disc pl-5 text-[13px]">
                                                <li class="pl-2">
                                                    <span class="inline-block align-top">This action cannot be undone, and the account(s) will be completely removed from the system.</span>
                                                </li>
                                                <li class="pl-2">
                                                    <span class="inline-block align-top">Any process or reports tied to this account(s) will be affected.</span>
                                                </li>
                                                <li class="pl-2">
                                                    <span class="inline-block align-top">Proceed only if you’re sure this account(s) should<br />be removed from access.</span>
                                                </li>
                                            </ul>
                                        </div>
                                ...
                                <div x-show="showConfirmRestoreModal" class="fixed inset-0 z-50 bg-gray-200 bg-opacity-50 flex justify-center items-center" @click.away="showConfirmRestoreModal = false"
                                    ...
                                    <div class="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full relative">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <h2 class="text-2xl font-extrabold text-blue-600 mb-2">Restore Account(s)</h2>
                                            <p class="text-sm text-zinc-700 text-center">You're going to restore the selected account(s) in the Recycle Bin table. Are you sure?</p>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('recycleBinHandler', () => ({
                ...
                toggleAll() {
                    ...
                    this.selectedRows = this.checkAll ? @json($trashedAccountantUsers->pluck('id')) : [];
                },

                bulkRestore() {
                    axios.post('{{ route("recycle-bin.accountant-users.bulkRestore") }}', { ids: this.selectedRows })
                        .then(response => location.reload())
                        .catch(error => console.error(error));
                },

                bulkDelete() {
                    axios.post('{{ route("recycle-bin.accountant-users.bulkDelete") }}', { ids: this.selectedRows, _method: "DELETE" })
                        .then(response => location.reload())
                        .catch(error => console.error(error));
                },
    ...
    function setEntries(entries) {
        ...
        form.method = 'GET';
        form.action = "{{ route('recycle-bin.accountant-users.index') }}";
