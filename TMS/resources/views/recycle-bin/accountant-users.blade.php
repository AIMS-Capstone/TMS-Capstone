                    ...
                    <div 
                        x-data="{
                            ...
                            toggleCheckbox(id) {
                                ...
                                this.checkAll = this.selectedRows.length === {{ json_encode($trashedAccountantUsers->count()) }};
                            },
                            toggleAll() {
                                this.checkAll = !this.checkAll;
                                if (this.checkAll) {
                                    this.selectedRows = {{ json_encode($trashedAccountantUsers->pluck('id')->toArray()) }}; 
                                } else {
                            ...
                            deleteRows() {
                                ...
                                fetch('{{ route('recycle-bin.accountant-users.bulkDelete' ) }}', {
                                    method: 'DELETE',
                                    headers: {
                            ...
                            restoreRows() {
                                ...
                                fetch('{{ route('recycle-bin.accountant-users.bulkRestore' ) }}', {
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/json',
                                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                                ...
                                <div x-data="recycleBinHandler">        
                                    ...
                                    <div class="flex flex-row items-center">
                                        <div class="relative w-80 p-5">
                                            <form x-target="accountant-table" action="/recycle-bin/accountant-users" role="search" aria-label="Table" autocomplete="off">
                                                <input 
                                                type="search" 
                                                name="search"
                                                        ...
                                                        <tr class="hover:bg-slate-100 cursor-pointer ease-in-out">
                                                            ...
                                                            <!-- Body Checkbox for Individual Selection -->
                                                                <label x-show="showCheckboxes" class="flex items-center cursor-pointer text-neutral-600" x-cloak>
                                                                    <div class="relative flex items-center">
                                                                        <input type="checkbox" @click="toggleCheckbox('{{ $AccountantUser->id }}')" :checked="selectedRows.includes('{{ $AccountantUser->id }}')" id="AccountantUser{{ $AccountantUser->id }}" 
                                                            ...
                                                            <td class="p-4">{{ \Carbon\Carbon::parse( $AccountantUser->deleted_at   )->format('F d, Y h:i A') ?? 'N/A' }}</td>
                                    ...
                                    <div 
                                        ...
                                        <div class="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full relative">
                                            <div class="flex flex-col items-center">
                                                ...
                                                <!-- Title -->
                                                <h2 class="text-2xl font-extrabold text-zinc-800 mb-2">Delete Account(s)</h2>

                                                <!-- Description -->
                                                <p class="text-sm text-zinc-600 text-center">
                                                    You're going to delete permanently the selected account(s) in the Accountant Recycle Bin table. Are you sure?
                                                </p>
