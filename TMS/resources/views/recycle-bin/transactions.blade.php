<x-organization-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="overflow-hidden sm:rounded-xs">
                ...
                <div class="overflow-x-auto pt-6 mt-4 px-10">
                    ...
                    <!-- Tabs Navigation -->
                    @php
                        $activeTab = request()->query('tab', 'transaction');
                    @endphp
                                                    ...
                                                    <tr class="hover:bg-slate-100 cursor-pointer ease-in-out">
                                                        ...
                                                        <td>
                                                            <button x-data 
                                                                ...
                                                                class="text-left text-xs py-4 px-4 hover:text-blue-600">
                                                        ...
                                                        <td class="py-3 px-5">
                                                            @if ($transaction->transaction_type == 'Sales') 
                                                                <span class="bg-zinc-100 text-zinc-800 text-xs font-medium me-2 px-4 py-2 rounded-full">Sales</span>
                                                            @elseif($transaction->transaction_type == 'Purchases')
                                                                <span class="bg-zinc-100 text-zinc-800 text-xs font-medium me-2 px-4 py-2 rounded-full">Purchases</span>
                                                            @else
                                                                <span class="bg-zinc-100 text-zinc-800 text-xs font-medium me-2 px-4 py-2 rounded-full">Manual Journal</span>
                                                            @endif
                                                        </td>
                                                        <td class="text-xs py-3 px-4">{{ \Carbon\Carbon::parse($transaction->deleted_at )->format('F d, Y h:i A') ?? 'N/A' }}</td>
                                                        <td class="py-3 px-4">{{ $transaction->deletedByUser->name ?? 'Unknown' }}</td>
                                                    </tr>
                                                @endforeach
                                        </tbody>
                                    </table>
                                    <div class="mt-4">
                                        {{ $trashedTransactions->links('vendor.pagination.custom') }}
                                    </div>
                                    @endif
                                </div>
                                    ...
                                    <div 
                                        ...
                                        <div class="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full relative">
                                            <div class="flex flex-col items-center">
                                                ...
                                                <!-- Title -->
                                                <h2 class="text-2xl font-extrabold text-zinc-800 mb-2">Delete Transaction(s)</h2>

                                                <!-- Description -->
                                                <p class="text-sm text-zinc-600 text-center">
                                                    You're going to delete permanently the selected transaction(s) in the Transaction Recycle Bin table. Are you sure?
                                                </p>
    ...
    function setEntries(entries) {
        ...
        form.method = 'GET';
        form.action = "{{ route('recycle-bin.transactions.index') }}";
