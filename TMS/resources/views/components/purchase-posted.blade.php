            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg" x-data="filterComponent()">
                <div class="container mx-auto my-4 pt-4">
                    <div class="flex justify-between items-center px-10">
                        <p class="font-bold text-3xl text-left taxuri-color">Purchase Book Journal</p>
                    </div>
                    <div class="flex justify-between items-center px-10">
                        ...
                        <div class="flex flex-row space-x-2 items-center justify-center">
                            <div x-data="{ selectedTab: 'Posted' }" class="w-full">
                                <div @keydown.right.prevent="$focus.wrap().next()" @keydown.left.prevent="$focus.wrap().previous()" class="flex justify-center gap-24 overflow-x-auto  border-neutral-300" role="tablist" aria-label="tab options">
                                    <a href="/purchase-book">
                                        <button @click="selectedTab = 'Draft'" :aria-selected="selectedTab === 'Draft'" 
                                            :tabindex="selectedTab === 'Draft' ? '0' : '-1'" 
                                            :class="selectedTab === 'Draft' ? 'font-bold text-blue-900' : 'text-neutral-600 font-medium hover:text-blue-900 hover:font-bold'" 
                                            class="h-min py-2 text-base relative" 
                                            type="button"
                                            role="tab" 
                                            aria-controls="tabpanelDraft" ><span class="block">Draft</span>
                                            <span 
                                                :class="selectedTab === 'Draft' ? 'block bg-blue-900 border-blue-900 border-b-4 w-[120%] rounded-b-md transform rotate-180 absolute bottom-0 left-[-10%]' : 'hidden'">
                                            </span>
                                        </button>
                                    </a>
                                    <button @click="selectedTab = 'Posted'" :aria-selected="selectedTab === 'Posted'" 
                                        :tabindex="selectedTab === 'Posted' ? '0' : '-1'" 
                                        :class="selectedTab === 'Posted' ? 'font-bold text-blue-900 ' : 'text-neutral-600 font-medium hover:text-blue-900 hover:font-bold'"
                                        class="h-min py-2 text-base relative" 
                                        type="button" 
                                        role="tab" 
                                        aria-controls="tabpanelPosted" ><span class="block">Posted</span>
                                        <span 
                                            :class="selectedTab === 'Posted' ? 'block bg-blue-900 border-blue-900 border-b-4 w-[120%] rounded-b-md transform rotate-180 absolute bottom-0 left-[-10%]' : 'hidden'">
                                        </span>
                                    </button>
                                </div>
                            </div>  
                    ...
                    <div
                        ...
                        <div class="container mx-auto">
                            <div class="flex flex-row space-x-2 items-center justify-between">
                                <div class="flex flex-row space-x-2 items-center ps-8">
                                    ...
                                    <div class="relative w-80 p-4">
                                        <form x-target="purchase-posted-table" action="/purchase-book/posted" method="GET" role="search" aria-label="Table" autocomplete="off">
                                            <input
                                                type="search"
                                                name="search"
                            ...
                            <!-- Table for Purchase Posted Transactions -->
                            <div x-data="{ checkAll: false, currentPage: 1, perPage: 5 }" class="mb-12 mt-6 mx-12 overflow-hidden max-w-full border-neutral-300" id="purchase-posted-table">
                                    ...
                                    <table class="w-full text-left text-sm text-neutral-600">
                                        <thead class="bg-neutral-100 text-sm text-neutral-700">
                                            <tr>
                                                ...
                                                <th scope="col" class="p-4">Customer Contact</th>
                                            ...
                                            @else
                                                <tr>
                                                    <td colspan="8" class="text-center p-6">
                                                        ...
                                                        <h1
                                                        <h1 class="font-extrabold mt-4">No Purchase Posted Transactions Available</h1>
                            ...
                            <div class="bg-white rounded-lg shadow-lg p-8 max-w-sm w-full relative" x-show="showConfirmUpdateModal" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100"
                                ...
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-sm text-zinc-600 text-center mb-6">
                                        You're going to mark as draft the selected item(s) in the Purchase Book Journal table.<br>Are you sure?
                                    </p>
                                    <!-- Actions -->
...
<script>
    ...
    return {
        ...
        async applyFilters() {
            ...
            .then(html => {
                ...
                const newTableContent = doc.querySelector('#purchase-posted-table').innerHTML;
                document.querySelector('#purchase-posted-table').innerHTML = newTableContent;
            })
    ...
    function exportReportExcel() {
        ...
        let url = `{{ route('purchase-posted.exportExcel') }}?year=${year}&month=${month}&period=${period}&quarter=${quarter}&start_month=${startMonth}&end_month=${endMonth}&status=${status}`;
    ...
    function setEntries(entries) {
        ...
        form.method = 'GET';
        form.action = "{{ route('purchasePosted') }}";
